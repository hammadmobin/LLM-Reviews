...
from src.config.config_deberta import (
    DEBERTA_MODEL_NAME,
...
def train_deberta(X, y):
    tokenizer = load_tokenizer(DEBERTA_MODEL_NAME)
    ...
    # tokenize dataset
    train_encodings = tokenizer(X_train, **DEBERTA_TOKENIZER_CONFIG)
    test_encodings = tokenizer(X_test, **DEBERTA_TOKENIZER_CONFIG)

    train_dataset = ReviewDataset(train_encodings, y_train)
    test_dataset = ReviewDataset(test_encodings, y_test)

    # model init
    model = load_classifier(DEBERTA_MODEL_NAME)

    training_args = TrainingArguments(**DEBERTA_TRAINING_ARGS)
